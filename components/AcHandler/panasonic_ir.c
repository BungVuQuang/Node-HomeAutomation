...
#define RMT_TX_CHANNEL RMT_CHANNEL_4 /*!< RMT channel for transmitter */
...
#define HEADER_MARK_US 3456	  /*!< Panasonic protocol header */
#define HEADER_SPACE_US 1728  /*!< Panasonic protocol header */
#define MARK_US 432			  /*!< Panasonic protocol mark */
#define BIT_ONE_SPACE_US 1296 /*!< Panasonic protocol space bit 1 */
#define BIT_ZERO_SPACE_US 432 /*!< Panasonic protocol space bit 0 */
#define IDLE_US 10000		  /*!< Panasonic protocol interframe spacing */
#define BIT_MARGIN 150		  /*!< Panasonic parse margin time */

// #define HEADER_MARK_US 3500	  /*!< Panasonic protocol header */
// #define HEADER_SPACE_US 1750  /*!< Panasonic protocol header */
// #define MARK_US 435			  /*!< Panasonic protocol mark */
// #define BIT_ONE_SPACE_US 1300 /*!< Panasonic protocol space bit 1 */
// #define BIT_ZERO_SPACE_US 435 /*!< Panasonic protocol space bit 0 */
// #define IDLE_US 10000		  /*!< Panasonic protocol interframe spacing */
// #define BIT_MARGIN 150		  /*!< Panasonic parse margin time */
...
//  */
static void fill_item_header3(rmt_item32_t *item)
{
...
{
	fill_item_level(item, HEADER_SPACE_US, HEADER_MARK_US);
}
...
static enum pana_item decode_item(const rmt_item32_t *item)
{
	...
	ESP_LOGI(TAG, "decode_item:  mark:%d,  space: %d", mark, space);
	if (space == 0)
	{
		return PANA_END;
	}
...
static void panasonic_transmit_frame(const uint8_t *data, int len)
{
	...
	int n = 0;
	ESP_LOGI(TAG, "ITEM SIZE: %d", 2 + sizeof(header) * 8 + 2 + len * 8 + 1);
	if (item == NULL)
	{
	...
	fill_item_header3(&item[n++]);
	// fill_item_level(&item[n++], MARK_US, MARK_US);
	...
	// 		printf(",");
	// 	}
	// 	printf("%i", item[i].duration0);
	// 	printf(",%i", item[i].duration1);
	// }
	// rmt_tx_start(RMT_TX_CHANNEL, true);
	rmt_write_items(RMT_TX_CHANNEL, item, n - 1, false);
	// vTaskDelay(2000 / portTICK_PERIOD_MS); // 5 sec delay
...
static void panasonic_rx_task()
{
	...
	while (1)
	{
		...
		const rmt_item32_t *item = (const rmt_item32_t *)xRingbufferReceive(rb, &rx_size, portMAX_DELAY);
		if (item)
		{
			int ret = 0;
			int length = rx_size / 4; // one RMT = 4 Bytes
			ESP_LOGE(TAG, "Rx_Size: %d", rx_size);
			for (const rmt_item32_t *i = item; rx_size >= sizeof(*i); i++, rx_size -= 4)
			{
				printf("  Channel 0 received %i items\n", length);
...
static void tx_init()
{
	...
	rmt_config(&rmt_tx);
	rmt_driver_install(rmt_tx.channel, 5000, 0);
}
